...
def BKNetModel(x):
    ...
    # primary branch
    prim_conv1 = VGG_ConvBlock('Block4', x, 128, 256, 3, 1, phase_train)
    prim_fc1 = _FC('prim_fc1', prim_conv1, 256, keep_prob)
    prim_fc2 = _FC('prim_fc2', prim_fc1, 256, keep_prob)
    y_prim_conv = _FC('prim_softmax', prim_fc2, 2, keep_prob, 'softmax')

    # secondary branch
    sec_conv1 = VGG_ConvBlock('Block5', x, 128, 256, 3, 1, phase_train)
    sec_fc1 = _FC('sec_fc1', sec_conv1, 256, keep_prob)
    sec_fc2 = _FC('sec_fc2', sec_fc1, 256, keep_prob)
    y_sec_conv = _FC('sec_softmax', sec_fc2, 3, keep_prob, 'softmax')

    return y_prim_conv, y_sec_conv, phase_train, keep_prob

def BKNetModel_variant2(x):
    phase_train = tf.placeholder(tf.bool)
    keep_prob = tf.placeholder(tf.float32)
    ...
    # primary branch
    prim_fc1 = _FC('prim_fc1', x, 256, keep_prob)
    prim_fc2 = _FC('prim_fc2', prim_fc1, 256, keep_prob)
    y_prim_conv = _FC('prim_softmax', prim_fc2, 2, keep_prob, 'softmax')

    # secondary branch
    sec_fc1 = _FC('sec_fc1', x, 256, keep_prob)
    sec_fc2 = _FC('sec_fc2', sec_fc1, 256, keep_prob)
    y_sec_conv = _FC('sec_softmax', sec_fc2, 3, keep_prob, 'softmax')

    return y_prim_conv, y_sec_conv, phase_train, keep_prob

def selective_loss(y_prim_conv, y_sec_conv, y_, mask):

    prim_mask = tf.cast(tf.equal(mask, 0.), tf.float32) #vector_zero
    sec_mask = tf.cast(tf.equal(mask, 1. ), tf.float32) #vector_one

    tf.add_to_collection('prim_mask', prim_mask)
    tf.add_to_collection('sec_mask', sec_mask)

    y_prim = tf.slice(y_, [0, 0], [-1, 2])
    y_sec = tf.slice(y_, [0, 0], [-1, 3])

    tf.add_to_collection('y_prim', y_prim)
    tf.add_to_collection('y_sec', y_sec)

    prim_cross_entropy = tf.reduce_sum(
        tf.reduce_sum(-y_prim * tf.log(y_prim_conv), axis=1) * prim_mask) / tf.clip_by_value(
        tf.reduce_sum(prim_mask), 1, 1e9)
    sec_cross_entropy = tf.reduce_sum(
        tf.reduce_sum(-y_sec * tf.log(y_sec_conv), axis=1) * sec_mask) / tf.clip_by_value(
        tf.reduce_sum(sec_mask), 1, 1e9)
    ...
    total_loss = prim_cross_entropy + sec_cross_entropy + l2_loss

    return prim_cross_entropy, sec_cross_entropy, l2_loss, total_loss
